...
//        4.2. Otherwise, authentication failed. Go back to authenticate.php
//////////////////////////////////////////////////////////////////////////////////
...
function authenticate() {
    if (isset($_SESSION[challenge]) &&
        ...
        if (validate($_SESSION[challenge], $_REQUEST[response], $password)) {
            ...
            header("Location:authenticate.php?error=" . urlencode("Failed authentication"));
            exit;
        }
    } else {
        header("Location:authenticate.php?error=" . urlencode("Session expired"));
        exit;
    }
}
